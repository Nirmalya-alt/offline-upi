...
public class ApiController {
    ...
    @GetMapping("/v1/wallet/details")
    public ResponseEntity<?> getWalletDetails(@RequestParam String upiId) {
        return walletRepo.findById(upiId)
                .map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }

    @GetMapping("/v1/wallet/transactions")
    public List<Transaction> getWalletTransactions(@RequestParam String upiId) {
        return txRepo.findBySenderVpaOrReceiverVpaOrderByIdDesc(upiId, upiId);
    }

    @PostMapping("/v1/payments/online")
    public ResponseEntity<?> onlinePayment(@RequestBody Map<String, String> req) {
        try {
            String sender = req.get("senderVpa");
            String receiver = req.get("receiverVpa");
            BigDecimal amount = new BigDecimal(req.get("amount"));
            String pin = req.get("pin");
            
            Transaction tx = settlement.onlineTransfer(sender, receiver, amount, pin);
